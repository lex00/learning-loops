// Renderer for "Truly parallel". Draws the marble run from the Sim; all vocabulary comes from the manifest.
...
const TEMPLATE = (m) => `
  ...
  <p class="ll-note"></p>
  <div class="ll-scene">
    <svg class="ll-svg" viewBox="80 8 520 324" role="img" aria-label="Marble run showing units of work that only compute, on one or two lanes, with a claw that can pull one off">
...
export function mount(host, M) {
  ...
  function drawSchedules() {
    ...
    for (const [key, sch] of Object.entries(M.schedules)) {
      const rows = M.expected[key];
      const names = M.variantOrder.filter(k => M.variants[k].schedule === key).map(k => M.variants[k].label);
      const wrap = document.createElement('div'); wrap.className = 'll-sched' + (key === V.schedule ? ' on' : '');
  ...
  function render(ev, mid = false) {
    ...
    const mark = m => [P.laneStart + P.beat * (M.skeleton[0].ticks - m.left), P.laneY[m.lane]];
    ...
    for (const e of ev) {
      ...
      if (e.type === 'run') {
        ...
        const at = mark(e.m), resumed = e.m.left < M.skeleton[0].ticks; raiseTo.set(e.m.dots, at);
